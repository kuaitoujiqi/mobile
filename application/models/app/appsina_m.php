...
class appsina_m extends CI_Model {
	...
	//托管充值   将旧接口替换掉
	function create_hosting_deposit_new($trade_no = false,$pay_type = array(),$monkey = false,$moblie = false,$uid){	
		$ip = '182.92.67.0';
		$post['payer_ip'] = $ip;
		$post['out_trade_no'] = $trade_no; 		//交易订单号
		$post['summary'] = "账户充值";  		//摘要
		$post['identity_id'] = $uid; //用户标识信息
		...
		if($moblie == "m")
		{
			$post['return_url'] = 'http://localhost:8080/x5/UI2/v_/APP/index.w#!main//(#!content20/!/content9/!/owerContent)';//同步跳转
		}
		else
		{
			$post['return_url'] = 'http://localhost:8080/x5/UI2/v_/APP/index.w#!main//(#!content20/!/content9/!/owerContent)';//同步跳转
	...
	{	
		...
		if($type == 2){
			$post['return_url'] = 'http://localhost:8080/x5/UI2/v_/APP/index.w#!main//(#!content20/!/content9/!/owerContent)';//异步跳转	
		}else{
			$post['return_url'] = 'http://localhost:8080/x5/UI2/v_/APP/index.w#!main//(#!content20/!/content9/!/owerContent)';//异步跳转	
		}
