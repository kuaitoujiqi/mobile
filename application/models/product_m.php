...
class Product_m extends CI_Model {
	...
	function new_bulk_buy($productid = false,$num = false,$type = 1)
	{
		...
		else
		{
			...
			$sina_return = $this->sina_m->create_hosting_collect_trade($order_id,$productid,$userinfo['id'],$user_monkey,$type);
			 ...
			 {
				$this->db->trans_commit(); 
				$this->db->set('content',$sina_return[1]);
				$this->db->set('time',time());
				$this->db->insert('sina_return_log');
	...
	function app_new_bulk_buy($productid = false,$num = false,$uid,$type = 1)
	{
		// $userinfo = userinfo();
		 // $uid = $uid;
		 // $uid = 500143;
		...
		{
			$order_id = $this->order_m->create_order(2,$uid,$user_monkey,false,$productid);
			$sina_return = $this->appsina_m->create_hosting_collect_trade($order_id,$productid,$uid,$user_monkey,$type);
			 if($sina_return[0] == 0)
			 {
				$this->db->trans_commit(); 
				$this->db->set('content',$sina_return[1]);
				$this->db->set('time',time());
				$this->db->insert('sina_return_log');
				return array('code'=>1,'msg'=>$sina_return); 
			 }
			 else
			 {
				 $this->db->trans_rollback();
